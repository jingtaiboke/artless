...
function normalize_paths() {
  ...
  $(ditto.content_id + " img").map(function() {
    var src = $(this).attr("src").replace("(index\\.html)?#/", "");
    if ($(this).attr("src").slice(0, 4) !== "http" && !$(this).attr("src").startsWith("/")) {
      ...
      var tmp = pathname + base_dir;
      if(pathname!=="" && base_dir!=="" && !pathname.endsWith("/") && !base_dir.startsWith("/")){
        tmp = pathname +"/"+ base_dir;
      }
      // normalize the path (i.e. make it absolute)
      $(this).attr("src", tmp + "/" + src);
    }else if($(this).attr("src").startsWith("/")){
      if(location.pathname.length>1){
          $(this).attr("src",location.pathname+src.substring(1));
      }
    }
  });
}
